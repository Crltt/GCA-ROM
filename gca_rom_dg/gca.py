...
class Encoder(torch.nn.Module):
    ...
    def __init__(self, hidden_channels, bottleneck, input_size, ffn, skip, act=F.elu):
        ...
        for i in range(self.depth-1):
            ...
            self.down_convs.append(GMMConv(self.hidden_channels[i], self.hidden_channels[i+1], dim=3, kernel_size=5, aggr='mean')) 
...
class Decoder(torch.nn.Module):
    ...
    def __init__(self, hidden_channels, bottleneck, input_size, ffn, skip, act=F.elu):
        ...
        for i in range(self.depth-1):
            self.up_convs.append(GMMConv(self.hidden_channels[self.depth-1-i], self.hidden_channels[self.depth-i-2], dim=3, kernel_size=5, aggr='mean')) 
